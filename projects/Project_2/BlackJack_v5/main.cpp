 ...
 * Created on June 3, 2019 2:45 PM
 ...
 *           another card or "Hit"
 *           Compare the totals of user and dealer to determine the winner
 */
...
void crtDeck(ofstream &,string,string);     //Create deck of 52 Cards
void dspRuls();                             //Displays Rules
...
int  stCrdVl(string [],int);                //Sets the value of the cards
...
int main(int argc, char** argv) {
    ...
    crtDeck(outFile,face,suit);
    outFile.close();    
    
    //Input cards into array
    inFile.open(flNmDk.c_str());    
    for(int i=0;i<52;i++){
        inFile>>deck[i];
    }
    inFile.close();
    ...
    if(answer=='Y'||answer=='y'){
        ...
        cout<<"Dealers total was "<<dlrSum<<endl;
        cout<<" "<<endl;
    }else if(dlrSum>usrSum && dlrSum<21){
...
void crtDeck(ofstream &outFile,string face,string suit){
    //Generate Cards
...
void dspRuls(){
    ...
    cout<<"If your total passes 21 then you bust and the dealer wins."<<endl;
    cout<<"And if your total and the dealers total are the same then, the dealer "<<endl;
    cout<<"automatically wins."<<endl<<endl;
